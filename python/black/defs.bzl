...
Add `rules_black` to your `MODULE.bazel` file

```python
bazel_dep(name = "rules_black", version = "{version}")
```

Next, the `rules_black` rules work mostly off of toolchains which are used to provide the necessary python targets (aka `black`)
